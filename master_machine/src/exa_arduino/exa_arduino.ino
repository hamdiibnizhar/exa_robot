...
byte  SPEED_MAX_SET_TO = 50;      //in percent
byte  PWM_MAX_DECIMAL = 255;
byte  PWM_MIN_DECIMAL = 0;
double  MAX_VALUE_CONTROLLER = 1;
double  MIN_VALUE_CONTROLLER = 0;

byte adj_speed = (byte)map(SPEED_MAX_SET_TO, PWM_MIN_DECIMAL, PWM_MAX_DECIMAL, SPEED_MIN, SPEED_MAX);
...
//if (param_x > -0.1 && param_x < 0) param_x = 0;
  ...
  byte vector_speed = (byte)map(vector_value(param_z, param_x), MIN_VALUE_CONTROLLER, MAX_VALUE_CONTROLLER, SPEED_MIN, adj_speed);

  if (param_x > min_x_value)
  {
    if (param_z > min_z_value)
    {
      turn_right(vector_speed);
    }
    if (param_z < -min_z_value)
    {
      turn_left(vector_speed);
    }
    if (param_z == 0)
    {
      run_forward(vector_speed);
    }
  }
  //reverse
  if (param_x < -min_x_value)
