...
std::string filenameData = "exa_robot_" + std::to_string(year) + "_" + std::to_string(month) + "_" + std::to_string(day) + "_" + std::to_string(hour) + "_" + std::to_string(minutes) + EXTENSION;
...
{
    //dataFile << xNow << "\t" << yNow << "\t" << zNow << "\t" <<  xTwist << "\t" << yTwist << "\t" << zTwist << "\t" << geiger_data_temp->data  << "\t" << distanceTemp << "\t" << timerun  << "\n";
#ifdef USE_CHRONO
...
{
    ros::init(argc, argv, "saving_data_robot");
