...
std::string filenameData = "exa_robot_loop1_" + std::to_string(year) + "_" + std::to_string(month) + "_" + std::to_string(day) + "_" + std::to_string(hour) + "_" + std::to_string(minutes) + EXTENSION;
...
{
    ros::init(argc, argv, "saving_data_robot_loop1");
    ros::NodeHandle n;
    dataFile.open (filenameData, std::fstream::out);
    
#ifdef USE_CHRONO
    auto start = std::chrono::high_resolution_clock::now();
    //auto start = std::chrono::high_resolution_clock::from_time_t(0);
#elif USE_ROS_TIME
    start = ros::Time::now();
#else
    start = std::clock();
